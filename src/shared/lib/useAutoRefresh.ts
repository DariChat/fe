 ...
 *
 * "새 방에 초대됨", "친구 요청 도착·수락" 은 이제 서버가 /user/queue 로 밀어준다
 * (useServerEvents). 그래서 이 훅은 더 이상 화면을 최신으로 만드는 주된 수단이 아니라
 * 소켓이 끊겨 있던 동안 놓친 변화를 메우는 안전망이다.
 *
 *  - 화면에 들어올 때 (첫 목록을 채우는 것도 이 호출이다)
 *  - 창이 다시 활성화될 때 — 절전·백그라운드로 소켓이 끊겼다 붙은 구간을 덮는다
 *  - 보고 있는 동안 아주 가끔
 *
